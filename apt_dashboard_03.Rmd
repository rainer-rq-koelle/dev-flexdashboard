...
asma_pm %>% filter(YEAR == max(YEAR)) %>%
  plot_ly( x   =~MONTH_NUM, y=~AVG_UNIMP_TIME, type = "bar"
          ,name="reference ASMA time") %>%
  add_trace( y   =~AVG_ADD_ASMA_TIME, type = "bar"
            ,name="additional ASMA time") %>%
  layout(barmode = "stack", xaxis = tick_yr_no_title)
```
...
subplot(g1,g2, shareY = TRUE
        ,nrows = 1, margin = 0.03, widths = c(0.4, 0.6)) %>%
...
some explanatory notes or more text to keep row column in sync.
...
# https://stackoverflow.com/questions/39948151/in-r-plotly-subplot-graph-how-to-show-only-one-legend
subplot( style(g1, showlegend=FALSE)
            , g2, nrows=1, shareY = TRUE, margin = 0.03, widths = c(0.35, 0.65)) %>%
...
# monthly trend ----------------------------------
dep_punc_pm <- params$punc %>% filter(SRC_PHASE == "DEP", YEAR == max(YEAR)) %>% 
  group_by(YEAR, MONTH, GROUP) %>% 
  summarise(N_GRP_MN = sum(N_GRP)) %>% ungroup() %>% 
  mutate(SHARE = N_GRP_MN / sum(N_GRP_MN)) 
...
plot_ly(labels=~SUBGROUP, values=~N) %>%
    add_pie(hole = 0.4)
```
