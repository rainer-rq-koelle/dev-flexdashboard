...
if(nrow(params$covid) < 1){
  cat("<p><b>This is under development!</b>")
}else{
...
yax = list(title= 'Average throughput [flights/hour]',titlefont = list(size = 11))

msg = paste0("<b>Average 15-min rolling hour <br>throughput-weekdays month May</b>")
...
thru %>% 
  ...
  add_annotations(text = msg
                 , x = "12:00"
                 , y = 0.1 * max(thru$ROLLING_HOUR_MVT) 
                 , showarrow = FALSE, ax = 0, ay = 0  # no arrow, no offset
                 , align = "center"
                 ) %>%
  layout( xaxis = xax, yaxis = yax
         ,hovermode = "x unified", hoverformat = ".2f") %>%
  config( displaylogo = FALSE
         ,modeBarButtonsToRemove = config_bar_remove_buttons)
```
...
if(nrow(worst_day_last_year) >= 5){
  ...
  key_msg <- paste0(
    ...
    ,"<br>(avg. Arrival ATFM Delay: ", worst_day_avgATFMdly, "min)</b>")
}
...
g1 <- txot_pa %>%
  ...
  layout(barmode = "stack", hovermode = "x unified", hoverformat = ".2f"
         , xaxis = list(title="")
...
turn_pm %>% 
  plot_ly() %>% 
  add_lines(x=~MONTH, y=~AVG_ATTT, color =~AC_CLASS, name = "actual"
          #https://github.com/ropensci/plotly/issues/1502
          #define mapping variable for transform filter, i.e. cannot link directly to ~YEAR!!!
          ,customdata=~YEAR
  ...
  add_lines(x=~MONTH, y=~AVG_STTT, color =~AC_CLASS, name = "scheduled", line = list(dash = 'dash')
            #https://github.com/ropensci/plotly/issues/1502
          #define mapping variable for transform filter, i.e. cannot link directly to ~YEAR!!!
          ,customdata=~YEAR
         # , type = "line" ## "bar"
...
### Variation of Annual Actual Turn-Around Times
...
turn_pm %>%
  ...
  layout( boxmode = "group"   # save to ignore warning message
         ,xaxis = list(title = "")
         ,yaxis = list( title = "Average actual turn-around time [min]"
                       ,titlefont = list(size = 11)
                       ,hoverformat = ".2f")
         ) %>%
  config( displaylogo = FALSE
