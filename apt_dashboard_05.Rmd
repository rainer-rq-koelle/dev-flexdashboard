...
id_tbl %>%
  datatable(
    options = list( # ---------------------------------
      ...
      ,  autoWidth = TRUE
      , columnDefs = list(list(width = '25px', targets = c(0))) # set 1st col
      , scrollX = TRUE
    ) # end options list ------------------------------
...
### COVID19 Snapshot Traffic Evolution
...
tmp <- actual_pm %>% left_join(sched_pm, by = c("YEAR", "MONTH"))

g2 <- tmp %>% filter(YEAR == max(YEAR)) %>%
  plot_ly(x = ~MONTH) %>%
  add_lines(y = ~ACT_H,  name = "actual Heavy") %>%
  add_lines(y = ~ACT_MJ, name = "actual Medium Jet") %>%
  add_lines(y = ~ACT_MT, name = "actual Medium Prop") %>%
  add_lines(y = ~SCH_H,  name = "scheduled Heavy"
           ,line = list(dash = "dash")) %>%
  add_lines(y = ~SCH_MJ, name = "scheduled Medium Jet"
            ,line = list(dash = "dash")) %>%
  add_lines(y = ~SCH_MT, name = "scheduled Medium Prop"
            ,line = list(dash = "dash")) %>%
  layout( xaxis = list(title = "", type = "category")
         ,yaxis = list(title = "average turntime [min]")
         )
